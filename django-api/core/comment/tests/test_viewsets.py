...
class TestCommentViewSet:
    ...
    def test_create(self, client, user, post):
        ...
        response = client.post(
            self.endpoint + str(post.public_id) + "/comment/",  data)
    ...
    def test_update(self, client, user, post, comment):
        ...
        response = client.put(
            self.endpoint + str(post.public_id) + "/comment/" + str(comment.public_id) + '/', data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data['body'] == data['body']
    ...
    def test_delete(self, client, user, post, comment):
        ...
        response = client.delete(self.endpoint +
                                 str(post.public_id) + "/comment/" + str(comment.public_id) + '/')
